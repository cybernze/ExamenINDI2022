...
//Iniciar capsa contenidora de escena
void MyGLWidget::creaCapsaContenidoraEscena(glm::vec3 pMin, glm::vec3 pMax)
{
    centreEsc = (pMax + pMin) / glm::vec3(2, 2, 2);
    radiEsc = sqrt(pow(pMax.x - pMin.x, 2) + pow(pMax.y - pMin.y, 2) + pow(pMax.z - pMin.z, 2)) / 2.0;
}
...
void MyGLWidget::iniEscena ()
{
  creaBuffersFantasma();
  creaBuffersCub();
  // radi interior de la roda + alçada cub --> 20+3
  // amplada de escena = 6 --> radi=3
  creaCapsaContenidoraEscena(glm::vec3(-23.0, -23.0, -3.0), glm::vec3(23.0, 23.0, 3.0));
  //Enviem la posicio del focus al shader
  enviaPosFocus();
...
{
  glm::mat4 TG(1.f);
  TG = glm::translate(TG, glm::vec3(0.0,21.0,posFantasma));
  TG=glm::rotate(TG,float(glm::radians(90.0)),glm::vec3(0,1,0));
  TG = glm::scale(TG, glm::vec3 (2.0f, 2.0f, 2.0f));
